 ...
 * `@nn-design/desktop/main`. The helpers are part of the security
...
describe('isHttpUrl', () => {
  ...
  it('rejects non-http schemes', () => {
    expect(isHttpUrl('nd://app/foo')).toBe(false);
...
describe('isAllowedChildWindowUrl (issue #911)', () => {
  it('allows the packaged nd:// scheme so live artifact previews open in a child BrowserWindow', () => {
    ...
    // In packaged builds the renderer lives at `nd://app/`, so that
    // relative href resolves to `nd://app/api/live-artifacts/.../preview?projectId=...`
    // by the time `setWindowOpenHandler` sees it.
    expect(isAllowedChildWindowUrl('nd://app/api/live-artifacts/abc/preview?projectId=p1')).toBe(true);
    expect(isAllowedChildWindowUrl('nd://app/')).toBe(true);
  });
...
describe('isAllowedEmbeddedBrowserUrl', () => {
  ...
  it('rejects executable or privileged schemes for embedded browser startup', () => {
    ...
    expect(isAllowedEmbeddedBrowserUrl('nd://app/')).toBe(false);
...
describe('resolveDesktopStatusUrl', () => {
  it('reports the pending URL while navigation is in flight', () => {
    expect(resolveDesktopStatusUrl(null, 'nd://app/')).toBe('nd://app/');
    expect(resolveDesktopStatusUrl('http://127.0.0.1:3000/', 'nd://app/')).toBe('nd://app/');
  });

  it('falls back to the last successful URL when no navigation is pending', () => {
    expect(resolveDesktopStatusUrl('nd://app/', null)).toBe('nd://app/');
