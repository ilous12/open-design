...
import { LAUNCHER_SCHEMA_VERSION, resolveLauncherVersionPaths } from "@nn-design/launcher-proto";
...
function fakeConfig(root: string, appVersion = "1.2.3-beta.4"): PackagedConfig {
  return {
    ...
    posthogHost: null,
    posthogKey: null,
    resourceRoot: join(root, "installed", "resources", "nn.design"),
...
describe("resolvePackagedLauncherRuntime", () => {
  ...
  it("uses the active launcher payload when runtime state and payload manifest are valid", async () => {
    ...
    try {
      ...
      await writeFile(join(resourcesPath, "nn.design", "bin", "node"), "");
      ...
      await writeFile(
        join(resourcesPath, "nn.design-config.json"),
        `${JSON.stringify({
      ...
      expect(runtime.config.webStandaloneRoot).toBe(join(resourcesPath, "nn.design-web-standalone"));
      expect(runtime.paths.resourceRoot).toBe(join(resourcesPath, "nn.design"));
