/**
 * Regression coverage for the `nd://` protocol proxy in
 * apps/packaged/src/protocol.ts.
 *
 * The packaged Electron entry registers `nd://` as the loader for the
...
describe('nd:// protocol proxy', () => {
  it('proxies the request through fetchImpl with the rewritten target URL', async () => {
    ...
    const request = new Request('nd://app/api/codex-pets/sync', { method: 'POST' });
  ...
  it('preserves the request path, search, and hash when rewriting to the web sidecar', async () => {
    ...
    const request = new Request('nd://app/api/projects?limit=5#section', { method: 'GET' });
  ...
  it('does not throw when fetch rejects (the actual #895 root-cause guard)', async () => {
    ...
    await expect(
      handleOdRequest(new Request('nd://app/'), 'http://127.0.0.1:1/', fetchImpl),
