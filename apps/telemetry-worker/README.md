# design for air Telemetry Relay

Cloudflare Worker relay for opt-in design for air telemetry. The shipped desktop
...
`POST /api/objects/batch`. It accepts design for air observability objects such as
...
The Worker requires the design for air telemetry marker header, validates the
...
```bash
pnpm --filter @nn-design/telemetry-worker deploy
```
