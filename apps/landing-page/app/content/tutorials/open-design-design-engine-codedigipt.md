---
title: 'Design For AIR — Turn the Agent You Already Have Into a Design Engine'
youtubeId: Z9_ruLqDJkM
summary: A practical guide to the part of Design For AIR most tutorials skip — choosing and swapping the model behind it. Your coding agent is the engine; the model you point it at is the quality knob. Install it, wire up Gemini CLI or Claude, and learn to match the model to the job. Based on Codedigipt's hands-on walkthrough.
date: 2026-05-03
category: Demo
durationSeconds: 924
author: Codedigipt
...
This guide focuses on the part of Design For AIR most walkthroughs gloss over: **the engine**. Design For AIR itself is the design surface — the real generation is done by whatever coding agent and model you point it at, and that choice is the single biggest lever on output quality. It follows the setup **Codedigipt** runs in [his hands-on walkthrough](https://www.youtube.com/watch?v=Z9_ruLqDJkM), rewritten and brought up to date with the current release. Watch the video above for the live run, or read on for the written version.

![The Design For AIR workspace — describe what you want and pick a mode.](/tutorials/open-design-design-engine-codedigipt/workspace-home.webp)
...
This is the step that matters. Open the settings (configure execution mode) and pick the agent that will drive generation from the CLIs Design For AIR detected on your machine. Anything not installed shows as disabled.
...
- **Anthropic API (BYOK)** — no CLI? Drop in an API key and pick the model directly. This lets you use Claude-quality output **without a separate Claude Design subscription** — you bring the key, Design For AIR brings the design surface.

## Step 3 — Generate, then judge by engine

Create a project, give it a name, choose **high fidelity**, and write your brief (Codedigipt builds a landing page for an "auth-as-a-service" API). Before it writes anything, Design For AIR returns a short **discovery form** — output format, primary surface, audience, visual tone, brand direction — so the agent locks the right direction instead of guessing. Answer it, pick a visual direction, and it builds.

![A real prototype generated in Design For AIR.](/tutorials/open-design-design-engine-codedigipt/generated-prototype.webp)
...
![The Design For AIR plugin library, with installable skills.](/tutorials/open-design-design-engine-codedigipt/plugin-library.webp)
...
Yes — it's the biggest lever. Design For AIR provides the skills and design systems, but the design judgement comes from the engine. The same brief on Opus vs a fast preview model produces noticeably different polish.

**Can I use Claude-level quality without paying for Claude Design?**
Yes. Point Design For AIR at Claude Code, or drop an Anthropic API key into settings (BYOK) and select the model. You get the output without a separate Claude Design subscription.
