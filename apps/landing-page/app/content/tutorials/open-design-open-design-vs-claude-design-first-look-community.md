---
title: 'design for air — An Honest First Look (Bugs, Fixes, and Any-Model Setup)'
youtubeId: lgwFdKAyaMM
summary: A warts-and-all first look at design for air — install it, configure any agent, build a real prototype live (rough edges included), use the on-canvas edit/inspect/comment tools, and wire up any model cheaply through an OpenAI-compatible provider like OpenRouter. Based on Sasha (ИИШНЫЙ)'s hands-on review.
date: 2026-05-07
category: Review
...
This guide is a deliberately honest first look at design for air — the parts that just work, and the rough edges you'll hit, shown live rather than edited out. It also covers a genuinely useful trick: wiring up *any* model cheaply through an OpenAI-compatible provider. It follows the hands-on review **Sasha (ИИШНЫЙ)** records in [his walkthrough](https://www.youtube.com/watch?v=lgwFdKAyaMM), rewritten in English and brought up to date with the current release. Watch the video above for the live run, or read on for the written version.

![The design for air workspace — describe what you want and pick a mode.](/tutorials/open-design-open-design-vs-claude-design-first-look-community/workspace-home.webp)
...
This is the most useful nugget in Sasha's review. Beyond the detected CLIs, design for air lets you **bring your own key for any OpenAI-compatible API** — which means you can route through an aggregator like OpenRouter and use almost any model, often more cheaply:

1. In your provider, create an API key and copy it.
2. In design for air settings, choose the **OpenAI** provider type (OpenRouter and similar aggregators speak the OpenAI API).
...
![The design for air plugins hub.](/tutorials/open-design-open-design-vs-claude-design-first-look-community/plugins-hub.webp)
...
Yes. Point design for air at any detected CLI, or bring your own key for any OpenAI-compatible provider (e.g. OpenRouter) by setting the key and base URL — that opens up almost any model.
