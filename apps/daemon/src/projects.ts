...
function buildDesignManifest(entries, projectLabel) {
  ...
  return JSON.stringify({
    ...
    title: projectLabel || 'Design For AIR project',
    entryFile,
    sourceFiles: {
      all: files,
      html: htmlFiles,
...
function buildDesignHandoff(entries, projectLabel) {
  ...
  return `# ${projectLabel || 'Design For AIR project'} implementation handoff
...
- Keep generated product UI free of Design For AIR chrome, preview labels, or design-process annotations.
...
- Do not keep prototype-only annotations, frame labels, or Design For AIR chrome in the production UI.
