...
// and free of @nn-design/sidecar-proto so it can be unit-tested
...
export interface BuildMcpInstallPayloadInputs {
  ...
  /** Browser-facing Design For AIR studio base URL (e.g.
...
export function buildMcpInstallPayload(
  ...
  if (!inputs.cliExists) {
    hints.push(
      `Design For AIR CLI entry is missing at ${inputs.cliPath}. Rebuild the daemon or packaged app and refresh.`,
    );
  }
  if (!inputs.nodeExists) {
    hints.push(
      `Node-compatible runtime at ${inputs.execPath} no longer exists. Reinstall Design For AIR or Node and restart the daemon.`,
    );
  }
