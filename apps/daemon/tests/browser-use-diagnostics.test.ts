...
describe('browser use diagnostics', () => {
  it('detects Browser Use prompts from the design for air Browser menu', () => {
    expect(isBrowserUseRequested('hello')).toBe(false);
    expect(isBrowserUseRequested('@agent-browser\n\nBrowser tab context:')).toBe(true);
    expect(isBrowserUseRequested('Use the selected design for air Browser tab as the bound target.')).toBe(true);
  });
