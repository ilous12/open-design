...
describe('composeSystemPrompt', () => {
  it('injects Korean as the hidden default language for local coding-agent prompts', () => {
    const prompt = composeSystemPrompt({});
  ...
  it('does not reintroduce task-type routing options under locale overrides', () => {
    const prompt = composeSystemPrompt({ locale: 'zh-CN' });

    expect(prompt).toContain('prototype is already the fixed route');
    expect(prompt).not.toContain('keep the `taskType` option labels as the canonical routing choices');
    expect(prompt).not.toContain('"id": "taskType"');
    expect(prompt).not.toContain('option labels as `原型`');
    expect(prompt).not.toContain('`实时作品`');
  });

  it('does not reintroduce task-type routing options for zh-TW locale overrides', () => {
