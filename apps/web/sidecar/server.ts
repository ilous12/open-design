...
function resolveWebRoot(): string {
  ...
  for (let depth = 0; depth < 8; depth += 1) {
    try {
      ...
      if (packageJson.name === "@nn-design/web") return current;
    } catch {
  ...
  throw new Error("failed to resolve @nn-design/web package root");
}
...
async function startStandaloneBackend(webRoot: string | null): Promise<StandaloneBackend> {
  ...
  if (entryPath == null) {
    throw new Error(
      webRoot == null
        ? `missing Next.js standalone server under ${WEB_STANDALONE_ROOT_ENV}; configure ${WEB_STANDALONE_ROOT_ENV} or install @nn-design/web`
