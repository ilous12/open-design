...
//
// Design For AIR connects to the configured servers as a CLIENT and surfaces
...
import type { TrackingExternalMcpElement } from '@nn-design/contracts/analytics';
import {
  disconnectMcpOAuth,
  fetchMcpOAuthStatus,
  fetchMcpServers,
...
// so the most useful categories for Design For AIR (visual generation, then
      ...
      {expanded ? (
        <>
          ...
          {isHttpLike && !row._isNew && row.id ? (
            usesManagedOAuth ? (
              ...
              <div className="mcp-oauth-hint hint">
                <strong>No managed OAuth.</strong> Design For AIR will use this
          ...
          {isHttpLike && row._isNew && usesManagedOAuth ? (
            <div className="mcp-oauth-hint hint">
              Save first, then click <strong>Connect</strong> to grant Design For AIR
              access via the provider's OAuth flow.
            </div>
          ) : null}
          {isHttpLike && row._isNew && !usesManagedOAuth ? (
            <div className="mcp-oauth-hint hint">
              <strong>No managed OAuth.</strong> Save this server and Design For AIR
        ...
        {connected ? (
          ...
          <>
            ...
            <span>
              ...
              <span className="hint">
                Click Connect to grant Design For AIR access via the provider's OAuth flow.
              </span>
            </span>
          </>
        )}
