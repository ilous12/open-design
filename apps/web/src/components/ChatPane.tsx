...
const CHAT_SCROLL_BOTTOM_EPSILON_PX = 24;
const CHAT_SCROLL_PINNED_THRESHOLD_PX = 56;
const CHAT_SCROLL_JUMP_BUTTON_THRESHOLD_PX = 140;

function defaultStarterPrompts(t: TranslateFn): StarterPrompt[] {
...
function isReferenceRemixProject(metadata: ProjectMetadata | undefined): boolean {
  if (!metadata) return false;
  if (typeof metadata.duplicatedFromReferenceRemixSlug === 'string' && metadata.duplicatedFromReferenceRemixSlug.trim()) {
    return true;
  }
  if (typeof metadata.duplicatedFromReferenceRemixEntry === 'string' && metadata.duplicatedFromReferenceRemixEntry.trim()) {
    return true;
  }
  return typeof metadata.templateId === 'string' && metadata.templateId.startsWith('reference-remix:');
}
...
export function ChatPane({
  ...
  hideNewConversationAction = false,
  conversations,
  ...
  // state below, which uses a wider threshold so the affordance stays
  // visible for a deliberate short scroll-up. We keep a small epsilon for
  // "already at bottom" so sub-pixel / layout jitter doesn't flash the
  // jump button when the user is effectively on the latest message.
  ...
  const handleNextStepPromptAction = useCallback((
    ...
    composerRef.current?.setDraft(prompt, {
      entryFrom: 'next_step',
      hiddenPrompt: options?.hiddenPrompt,
  ...
  const starterExamples = useMemo(() => defaultStarterPrompts(t), [t]);
  const showStarterExamples =
    (Boolean(initialDraft?.trim()) || isReferenceRemixProject(projectMetadata)) &&
    ...
    if (saved !== null) {
      requestAnimationFrame(() => {
        ...
        setScrolledFromBottom(
          distance > CHAT_SCROLL_JUMP_BUTTON_THRESHOLD_PX + CHAT_SCROLL_BOTTOM_EPSILON_PX,
        );
        pinnedToBottomRef.current =
          distance <= CHAT_SCROLL_PINNED_THRESHOLD_PX + CHAT_SCROLL_BOTTOM_EPSILON_PX;
      });
    }
          ...
          {showConvList ? (
            <div className="chat-history-menu" role="menu" data-testid="conversation-history-menu">
              <div className="chat-history-menu-head">
                ...
                {onNewConversation && !hideNewConversationAction ? (
                  <button
                    type="button"
                          ...
                          {starterExamples.map((example, index) => (
                            <button
                              ...
                              onClick={() => {
                                ...
                                composerRef.current?.setDraft(example.prompt);
                              }}
                            ...
                            >
                              ...
                              <span className="chat-example-body">
                                <span className="chat-example-head">
                                  <span className="chat-example-title">{example.title}</span>
                                  <span className="chat-example-tag">{example.tag}</span>
                                </span>
                                <span className="chat-example-prompt">{example.prompt}</span>
                              </span>
