...
describe('social-share contract', () => {
  it('builds Design For AIR repository share targets', () => {
    const payload = buildSocialSharePayload({
      kind: 'open-design-repo',
      locale: 'zh-CN',
      title: 'Design For AIR GitHub',
      text: '推荐 Design For AIR',
    });
  ...
  it('keeps deployed project links and the repo recommendation together', () => {
    const payload = buildSocialSharePayload({
      ...
      title: 'Demo',
      text: `Built with Design For AIR. Repo: ${OPEN_DESIGN_GITHUB_REPO_URL}`,
