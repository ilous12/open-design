...
// bundled skill+template (web prototypes) point to the
...
// re-inventing every section's CSS from scratch.
//
...
export const DEFAULT_SCENARIO_PLUGIN_BY_KIND: Record<ProjectKind, DefaultScenarioPluginId> = {
  ...
  // Deck templates are no longer bundled; route through the generic
  // generation scenario unless the user explicitly installs one.
...
export function defaultScenarioPluginIdForProjectMetadata(
  ...
  if (metadata?.intent === 'live-artifact') return 'od-new-generation';
