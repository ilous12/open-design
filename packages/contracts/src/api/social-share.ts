...
export function buildSocialSharePayload(input: SocialShareRequest): SocialShareResponse {
  ...
  const fallbackTitle = kind === 'project-html' ? 'Design For AIR project' : 'Design For AIR';
  const title = cleanText(input.title, fallbackTitle);
  const fallbackText = kind === 'project-html'
    ? `Built with Design For AIR: ${title}. Design For AIR repo: ${OPEN_DESIGN_GITHUB_REPO_URL}`
    : 'Design For AIR is an open-source workspace for creating, editing, deploying, and handing off design artifacts.';
