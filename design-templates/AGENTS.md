...
renders into a project artifact. The active bundled catalogue is limited to
prototypes, wireframes, and mobile app templates. Each entry is a folder with
a `SKILL.md` (same shape as functional skills) plus rendering side files
(`example.html`, `assets/`, `references/`, …).
...
1. Create `design-templates/<my-template>/SKILL.md` with `name`,
   `description`, `triggers`, and an explicit `od.mode` that fits the active
   bundled catalogue (`prototype` for prototype, wireframe, and mobile app
   templates).
