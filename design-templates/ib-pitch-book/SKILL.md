...
repackaged as an Design For AIR `deck` skill.
...
Follow Design For AIR's deck export path for the active session (HTML / PDF /
PPTX per daemon capabilities).

## Relationship to Design For AIR financial skills
...
original adaptation for Design For AIR.
