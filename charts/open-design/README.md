<!--- app-name: Design For AIR -->

## Introduction
This chart bootstraps an [Design For AIR](https://github.com/nexu-io/open-design) deployment on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
These commands deploy an Design For AIR application on the Kubernetes cluster in the default configuration.
...
The current Design For AIR runtime stores state in local files and SQLite. Before
...
Design For AIR relies on Server-Sent Events (SSE) for real-time streaming. If you enable the Ingress resource, it is critical to disable reverse-proxy buffering. If you are using the NGINX Ingress Controller, this chart automatically applies the required annotations by default:
...
| `image.repository` | Design For AIR image repository              | `ghcr.io/nexu-io/od`        |
...
The above command sets the Design For AIR node environment to `development`.
