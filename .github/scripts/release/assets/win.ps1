...
$sourceInstaller = Join-Path $builderDir "Design For AIR-${env:TOOLS_PACK_NAMESPACE}-setup.exe"
$sourceZip = Join-Path $builderDir "Design For AIR-${env:TOOLS_PACK_NAMESPACE}-portable.zip"
...
$releaseNotes = if ([string]::IsNullOrWhiteSpace($env:RELEASE_NOTES)) {
  "Design For AIR ${env:RELEASE_VERSION}$assetSuffix"
} else {
  $env:RELEASE_NOTES
}
