...
checked into the Design For AIR repo (rather than pulled in as a Git submodule
or an npm package) so that:

- Any Design For AIR agent can run the skill end-to-end without a network
...
- Pinned upstream reference (declared in `SKILL.md` frontmatter): see the
  ...
  directory as the authoritative source-of-truth for Design For AIR and
...
2. Replace the contents of this directory with the upstream snapshot,
   ...
   inside `SKILL.md`'s `> **Design For AIR integration.**` blockquote.
...
flow (including how Design For AIR behaves when Codex is not installed).
