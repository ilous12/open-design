...
description: |
  ...
  Design For AIR browser-tab context, testing web apps, dogfooding Design For AIR
  previews, QA, bug hunts, or reviewing app quality. Prefer local Design For AIR
...
Use `agent-browser` for local Design For AIR preview validation: inspect rendered
...
For selected Design For AIR browser tabs and browser-use/browser-harness-style
...
5. If the selected Design For AIR context only provided a URL/title and no browser
...
Do not run Design For AIR's own daemon CLI as a browser automation tool. Commands
...
With the Design For AIR preview at `http://127.0.0.1:17573/`, run:
...
Expected success: title `Design For AIR`, current URL under `127.0.0.1:17573`,
visible Design For AIR UI text in the snapshot, and a screenshot at
