...
async function writeExtractedWindowsPayload(destinationRoot: string, namespace: string): Promise<void> {
  ...
  await writeFile(
    join(destinationRoot, "payload", "resources", "nn.design-config.json"),
    `${JSON.stringify({
...
async function writeExtractedMacPayload(destinationRoot: string, namespace: string): Promise<void> {
  const resourcesRoot = join(destinationRoot, "payload", "Design For AIR Beta.app", "Contents", "Resources");
...
const platformCases: PlatformCase[] = [
  ...
  },
  {
    ...
    currentVersion: "1.2.3-beta.4",
    expectedResourceRoot: (root, namespace) =>
      join(root, "launcher", "channels", "beta", "namespaces", namespace, "versions", "1.2.3-beta.5", "payload", "Design For AIR Beta.app", "Contents", "Resources", "open-design"),
