...
release/staging/mac/             mac release staging
...
Use this only for local VM testing without a signing key. It keeps the same handoff shape but skips Authenticode validation.

```text
powershell -ExecutionPolicy Bypass -File .\release\release_win_1_build_app.ps1

Copy-Item .\release\signing\to-external\app-signing-request.zip `
  .\release\signing\from-external\signed-app.zip -Force

powershell -ExecutionPolicy Bypass -File .\release\release_win_2_after_app_signed_build_installer.ps1 `
  -SignedAppZip .\release\signing\from-external\signed-app.zip `
  -AllowUnsignedTest

Copy-Item .\release\signing\to-external\installer-signing-request.zip `
  .\release\signing\from-external\signed-installer.zip -Force
