...
- 💬 **Keep talking while the model is still working.** Queue chat sends mid-stream, then let Design For AIR continue the conversation the moment the current turn finishes. Studio and Draw now follow the same flow, so capturing an idea does not depend on waiting for the previous response to end. (#2870, #3270, #1961) Thanks @zoeforfun, @lefarcen, @leno23.
...
- 📦 **Trying Design For AIR gets easier on Windows and Linux.** Windows gets a portable zip path; Linux gets Docker/Podman Compose one-click setup. Less install friction, faster first run. (#2937, #2414) Thanks @PerishCode, @epicsagas.
- 🔌 **MCP clients can now do real workspace work.** Write files, delete files, delete projects, resolve the active project directory, run generation loops, and bootstrap Codex from one place. External clients can now participate in the Design For AIR workspace instead of only observing it. (#2416, #2802, #3141) Thanks @YOMXXX, @papperrollinggery, @lefarcen.
- 🛂 **Network changes stop requiring a restart.** Switch the system proxy while Design For AIR is running; the next outbound request picks it up automatically. (#3093) Thanks @mrcfps.
...
#### 🤖 Agents & runtimes
- **Design For AIR AMR (vela) as a first-class ACP stdio agent**, with onboarding, sign-in pill, and bundled `vela` CLI inside the installer. (#2355) Thanks @lefarcen.
...
- **Refresh Design For AIR app visuals** — new app icons, logo, brand glyphs. ([#2436])
...
- Shared `CritiqueRoundSummary` / `CritiqueRunStatus` types via the `@nn-design/contracts` package. ([#1016])
...
- Allow `nd://` URLs through `setWindowOpenHandler` so live-artifact previews open in a child window. ([#933])
...
- Pack/win: close detection gaps that let `Design For AIR.exe` stay locked at install time. ([#823])
...
- Release: Design For AIR 0.5.0 changelog landing. ([#820])
...
#### Packaging
- Fixed packaged desktop startup by building `@nn-design/contracts` to `dist/*.mjs` + `.d.ts`, pointing its exports at compiled JavaScript, and building contracts before all packaged lanes pack workspace tarballs. ([#577])
...
A multi-protocol leap: Design For AIR now ships as an MCP server, ships Critique Theater (Design Jury) Phase 4, gains live-reload + Tweaks mode + live artifacts in the preview pane, and adds five new agent / runtime adapters. 71 merged PRs from 40+ contributors over two days. Linux AppImage packaging landed in tooling, but the stable Linux artifact is deferred from 0.4.0 while containerized release packaging is hardened.

### Added

#### MCP & agent integration
- **`od mcp` — expose Design For AIR as a stdio MCP server.** Coding agents in other repos (Claude Code, Codex, Cursor, VS Code, Antigravity, Zed, Windsurf) can read files from local Design For AIR projects directly, including the project the user has open in the Design For AIR app right now. ([#399])
...
First public release of Design For AIR — a local-first, open-source alternative to Anthropic's Claude Design. It detects your installed code-agent CLI, runs design skills against curated design systems, and streams artifacts into a sandboxed in-app preview.
...
- Initial project structure, project rename "Open Claude Design" → "Design For AIR", naming optimization. ([#1], [#2])
