...
- a tokenized HTML template (`template.html`) wired entirely with Design For AIR's scalar-only `{{data.X}}` bindings;
- a default sample `data.json` covering a 14-account month-over-month OTD slice plus a "lowest 8" deep-dive;
- the canonical `artifact.json` and `provenance.json` shapes the Design For AIR daemon expects;
...
Design For AIR's `html_template_v1` renderer is intentionally narrow:
