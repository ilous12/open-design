// design for air web clipper on-page UI.
//
...
//      led by the design for air brand mark. HIDDEN by default — turned on from
...
(function () {
  ...
  // no message listener: the popup then reported "design for air hasn't attached to
  ...
  async function commitCapture(el) {
    ...
    try {
      ...
      console.warn('[design for air] element capture failed', err);
      stopBusy();
      toast(t('extensionErrorReload'));
      return;
    } finally {
