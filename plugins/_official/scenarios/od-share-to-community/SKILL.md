...
description: Package the user's just-finished work as an Design For AIR plugin without asking for fields the project files already answer, then surface the existing Add-to-My-plugins / Open-Design-PR buttons.
od:
...
Triggered by the post-completion "Share to Design For AIR" submission action. The user just finished a piece of work in this project and wants to ship it as a plugin. They have not been asked any questions yet.
...
- **Design For AIR PR** — opens a draft PR against `nexu-io/open-design` for the community catalog.
