...
description: >
  ...
  collage) — the same aesthetic Design For AIR uses for its own marketing
...
This is the canonical Design For AIR marketing-page recipe — the example
...
worked example (Design For AIR itself).
...
`pnpm --filter @nn-design/landing-page build` for a static `out/`
...
├── inputs.example.json      # Design For AIR as the worked example
...
└── assets/
    ├── *.png                # 16 collage plates (Design For AIR instance)
