...
description: Open a pull request adding a local Design For AIR plugin to the Design For AIR community catalog using gh CLI.
triggers:
  - contribute plugin
  - open design pr
  - github pull request
...
# Contribute Plugin to Design For AIR

Use this workflow when the active project contains a copied plugin folder and the user wants to propose it for the Design For AIR community catalog.
...
Keep the pull request focused. Do not modify unrelated Design For AIR files unless a manifest validation issue requires a tiny supporting change.
