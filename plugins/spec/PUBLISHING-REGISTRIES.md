...
Design For AIR plugins are intentionally shaped so one folder can travel across multiple agent ecosystems. The safest publishing model is:

1. Keep the source of truth in a public GitHub repository or in an Design For AIR PR.
2. Keep `SKILL.md` portable and registry-friendly.
3. Add `open-design.json` as the Design For AIR sidecar.
...
2. Push a public GitHub repository or open an Design For AIR PR.
3. Add README install instructions for Design For AIR and generic Agent Skills clients.
...
| --- | --- | --- | --- |
| Design For AIR | OD marketplace, composer chips, pipelines, GenUI, artifact provenance | `SKILL.md` + `open-design.json` | Open a PR to Design For AIR or publish a marketplace index entry pointing to the plugin repo. |
...
For Design For AIR plugin authors:
...
Use this path for normal Design For AIR plugins because they are centered on `SKILL.md`.
...
- Link back to the canonical GitHub repo and Design For AIR PR.
