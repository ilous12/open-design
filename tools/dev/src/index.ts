...
import { createSidecarLaunchEnv, requestJsonIpc } from "@nn-design/sidecar";
import {
...
async function buildDesktop(config: ToolDevConfig, logHandle: FileHandle): Promise<void> {
  await logHandle.write(`\n[tools-dev] building @nn-design/desktop at ${new Date().toISOString()}\n`);
  const invocation = createPackageManagerInvocation(["--filter", "@nn-design/desktop", "build"], process.env);
...
async function runForeground(config: ToolDevConfig, appName: string | undefined, options: CliOptions) {
  ...
  await new Promise<void>((resolveDone) => {
    const shutdown = () => {
      ...
      process.stderr.write("\nStopping Design For AIR dev server...\n");
...
function addPortOptions(command: ReturnType<typeof cli.command>) {
  return command
    ...
    .option("--prod", "use production build (requires pnpm --filter @nn-design/web build first)");
}
