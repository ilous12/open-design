...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Design For AIR-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Design For AIR-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Design For AIR"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Design For AIR", "Design For AIR.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Design For AIR-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Design For AIR-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Design For AIR-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Design For AIR", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "Design For AIR.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
        join(cachedUnpackedRoot, "resources", "nn.design-config.json"),
      ...
      await writeFile(
        ...
        `${JSON.stringify({ name: "nn.design-packaged-app", version: "0.5.0-beta.1" })}\n`,
        "utf8",
      );
      ...
      await expect(readFile(join(paths.unpackedRoot, "resources", "nn.design-config.json"), "utf8")).resolves.toContain(
        '"namespace":"second"',
      );
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "Design For AIR.exe"))).resolves.toMatchObject({
...
async function createVersionedExecutable(packagedVersion: string): Promise<Buffer> {
  ...
  version.setStringValues(
    ...
    {
      FileDescription: "Design For AIR",
