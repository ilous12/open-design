...
import { LAUNCHER_SCHEMA_VERSION } from "@nn-design/launcher-proto";
...
async function writeFakeMacApp(config: ToolPackConfig): Promise<ReturnType<typeof resolveMacPaths>> {
  ...
  await mkdir(join(resourcesRoot, "nn.design", "bin"), { recursive: true });
  await mkdir(join(resourcesRoot, "nn.design", "prebundled", "daemon"), { recursive: true });
  await mkdir(join(resourcesRoot, "nn.design", "prebundled", "web"), { recursive: true });
  await mkdir(join(resourcesRoot, "nn.design-web-standalone"), { recursive: true });
  await writeFile(executablePath, "#!/bin/sh\nexit 0\n", "utf8");
  await chmod(executablePath, 0o755);
  await writeFile(join(resourcesRoot, "nn.design", "bin", "node"), "#!/bin/sh\nexit 0\n", "utf8");
  await writeFile(join(resourcesRoot, "nn.design", "prebundled", "daemon", "daemon-sidecar.mjs"), "export {};\n", "utf8");
  await writeFile(join(resourcesRoot, "nn.design", "prebundled", "web", "web-sidecar.mjs"), "export {};\n", "utf8");
  await writeFile(
    join(resourcesRoot, "nn.design-config.json"),
    `${JSON.stringify({
...
async function writeFakeWinUnpackedApp(root: string, namespace: string, version: string): Promise<{
  ...
  await writeFile(
    ...
    `${JSON.stringify({ name: "nn.design-packaged-app", version })}\n`,
    "utf8",
  );
  ...
  it.skipIf(process.platform !== "win32")("reuses the Windows payload archive when base and overlay metadata are unchanged", async () => {
    ...
    try {
      ...
      const config = JSON.parse(
        await readFile(join(extractRoot, "payload", "resources", "nn.design-config.json"), "utf8"),
