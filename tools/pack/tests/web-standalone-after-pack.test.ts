...
async function writeRootWebPackage(resourcesRoot: string): Promise<void> {
  const webPackageRoot = join(resourcesRoot, "app", "node_modules", "@nn-design", "web");
  await mkdir(join(webPackageRoot, "dist", "sidecar"), { recursive: true });
  await writeFile(join(webPackageRoot, "package.json"), "{\"name\":\"@nn-design/web\"}\n", "utf8");
...
async function runFixture(options: {
  ...
  const root = await mkdtemp(join(tmpdir(), "nn.design-web-standalone-hook-"));
  ...
  const resourcesRoot = platformName === "darwin"
    ? join(appOutDir, "Design For AIR.app", "Contents", "Resources")
    : join(appOutDir, "resources");
  const appPath = join(appOutDir, "Design For AIR.app");
  ...
  if (platformName === "darwin" && options.writeMacCodeBundleFixture) {
    ...
    await mkdir(join(frameworksRoot, "Design For AIR Helper.app"), { recursive: true });
  }
    ...
    `${JSON.stringify(
      {
        ...
        resourceName: "nn.design-web-standalone",
  ...
  try {
    await runWebStandaloneAfterPack({
      ...
      packager: { appInfo: { productFilename: "Design For AIR" } },
    });
  } catch (error) {
  ...
  return {
    appOutDir,
    auditReportPath,
    destinationRoot: join(resourcesRoot, "nn.design-web-standalone"),
    root,
  };
}
...
describe("web standalone afterPack hook", () => {
  ...
  darwinSymlinkIt("rewrites darwin copied pnpm symlinks to stay inside the packaged resource", async () => {
    ...
    try {
      ...
      expect(report.copiedAudit.resolvedModules["next/package.json"].split(path.sep).join("/")).toMatch(
        /nn.design-web-standalone\/node_modules\/\.pnpm\/next@0\.0\.0\/node_modules\/next\/package\.json$/,
      );
    } finally {
      await rm(fixture.root, { force: true, recursive: true });
    }
    ...
    try {
      ...
      expect(signedTargets).toEqual(
        expect.arrayContaining([
          ...
          expect.stringMatching(/Design For AIR Helper\.app$/),
          expect.stringMatching(/Design For AIR\.app$/),
        ]),
      );
