...
export async function collectMacSizeReport(
  ...
  return {
    ...
    tracked: {
      ...
      webNextCacheBytes: await sizePathBytes(join(appNodeModulesRoot, "@nn-design", "web", ".next", "cache")),
      webPackageBytes: await sizePathBytes(join(appNodeModulesRoot, "@nn-design", "web")),
      webPackageStandaloneBytes: await sizePathBytes(join(appNodeModulesRoot, "@nn-design", "web", ".next", "standalone")),
    },
