...
async function runElectronBuilderRaw(
  ...
  const builderConfig = {
    ...
    productName: PRODUCT_NAME,
    publish: [{ provider: "generic", url: "https://updates.invalid/design-for-air" }],
    win: {
...
async function assertMaterializedUnpackedVersionConsistency(
  ...
  const packagedConfigPath = join(unpackedRoot, "resources", "nn.design-config.json");
      ...
      const materializedManifest = await cache.readHit({
        materialize: [{
          ...
          reuseRequiredPaths: [
            ...resolveWinNsisOverlayRequiredPaths(),
            [
              "resources/nn.design-web-standalone/apps/web/server.js",
              "resources/nn.design-web-standalone/server.js",
            ],
          ],
