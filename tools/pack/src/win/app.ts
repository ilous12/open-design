...
import { createCommandInvocation, createPackageManagerInvocation } from "@nn-design/platform";
...
async function runEsbuild(config: ToolPackConfig, args: string[]): Promise<void> {
  await runPnpm(config, ["--filter", "@nn-design/packaged", "exec", "esbuild", ...args]);
}
...
async function buildWorkspaceArtifacts(config: ToolPackConfig): Promise<void> {
  ...
  try {
    await runPnpm(config, ["--filter", "@nn-design/web", "build"], { OD_WEB_OUTPUT_MODE: config.webOutputMode });
