...
- `tools/dev` provides `@nn-design/tools-dev` and the `tools-dev` bin. It is the only currently active local development lifecycle control plane.
...
- `tools/pack` provides `@nn-design/tools-pack` and the `tools-pack` bin. The active slice is packaged artifact build/install/start/stop/logs/uninstall/cleanup/list/reset plus beta release artifact preparation for mac and Windows lanes, plus a Linux AppImage lane with optional containerized builds.
- `tools/serve` provides `@nn-design/tools-serve` and the `tools-serve` bin. It owns local fixture services such as `tools-serve start updater`.

## Retired tools

- `tools/pr` / `@nn-design/tools-pr` / `pnpm tools-pr` has been retired from this repository. Maintainer PR-duty workflows now live outside the product workspace in `PerishCode/duty`; do not restore an design for air-local PR-duty tool without a new explicit maintainer decision.
...
- Orchestration layers must consume primitives from `@nn-design/sidecar-proto`, `@nn-design/sidecar`, and `@nn-design/platform`.
...
```bash
pnpm --filter @nn-design/tools-dev typecheck
