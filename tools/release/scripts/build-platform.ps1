...
function Validate-WinLauncherPayloadArchive([string]$PayloadPath, [string]$ExpectedVersion, [string]$Label) {
  ...
  try {
    ...
    Test-JsonString $manifest.entry.executable "entry.executable" "payload/design for air.exe"
    ...
    $configPath = Join-Path $extractRoot (Convert-ArchiveRelativePath "payload/resources/nn.design-config.json")
