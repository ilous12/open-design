...
case "$RELEASE_TARGET" in
  mac_arm64 | mac_x64)
    artifact_mode="${RELEASE_ARTIFACT_MODE:-dmg-and-payload}"
    ...
    release_notes="${RELEASE_NOTES:-design for air $RELEASE_VERSION$RELEASE_ASSET_SUFFIX}"
...
EOF
    ;;
  linux_x64)
    source_appimage="$TOOLS_PACK_DIR/out/linux/namespaces/$RELEASE_NAMESPACE/builder/design for air-$RELEASE_NAMESPACE.AppImage"
