...
> **Quick start for contributors:** This guide helps you add a new language translation to Design For AIR in ~2 hours instead of ~8 hours. Follow the checklist, avoid common mistakes, and ship your PR with confidence.
...
- ✅ Alt text: `alt="Design For AIR banner"`
...
- ❌ Brand names: "Design For AIR", "Claude Code"
...
**Don't forget to update test fixtures:** Add your locale code to `EXPECTED_LOCALES` in `apps/web/tests/i18n/locales.test.ts` and add a `LOCALE_LABEL` assertion (e.g., `expect(LOCALE_LABEL.it).toBe('Italiano');`). Run `pnpm --filter @nn-design/web test` to verify.
...
Design For AIR currently supports **19 languages** across different surfaces:
...
9. **Run checks:**
   ...
   pnpm --filter @nn-design/web test  # Covers locale/content drift tests
   ```
...
- ✅ Alt text in images: `alt="Design For AIR banner"` → `alt="Banner di Design For AIR"`
...
- ❌ Brand names: "Design For AIR", "Claude Code", "Anthropic", "Vercel"
...
- Use the English term with a brief explanation in parentheses on first use if no standard translation exists:
  ```
  Design For AIR è un'alternativa open-source (codice aperto) a Claude Design.
  ```
...
- Design For AIR, Claude Code, Claude Design
...
- `Skill` stays `Skill` when it names the Design For AIR/Claude skill format.
  Translate only generic prose such as "ability" or "capability" as
  `capacité`.
- `fork` stays `fork` when it names the Design For AIR conversation-fork feature
...
- `SOTA Harness` and `Harness` stay in English when they name the Design For AIR
...
**Example:**
```markdown
Design For AIR هو البديل مفتوح المصدر لـ Claude Design
```
...
```markdown
## Summary
Adds [Language] translation for Design For AIR documentation.
...
Every translation makes Design For AIR accessible to more developers worldwide.
...
**Ready to contribute?** Pick a language, follow the [Quick Start](#-quick-start-adding-your-language-in-5-steps), and submit your PR. We can't wait to see Design For AIR in your language! 🚀
