...
- The repository prefers package-scoped validation, including `pnpm --filter @nn-design/web test`. Source: `AGENTS.md#Common commands`.
...
- Run `pnpm --filter @nn-design/web test` after adding the test. Source: `AGENTS.md#Common commands`.
...
- Passed: `pnpm --filter @nn-design/web test -- tests/components/assistant-message-tool-status.test.tsx tests/runtime/tool-renderers.test.tsx` (`84` files, `751` tests).
- Passed: `pnpm --filter @nn-design/web typecheck`.
