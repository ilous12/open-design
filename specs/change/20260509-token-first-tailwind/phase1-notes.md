...
### Step 2: Design For AIR Tailwind tokens
...
- `scripts/guard.ts` - added default Tailwind palette utility rejection for classes such as `text-red-500`, `bg-white`, `border-zinc-200`, `from-orange-500`, and related color utility namespaces so app UI uses Design For AIR token utilities.
...
- Style guard allowlist and promotion policy:
  ...
  | Repeated arbitrary app UI colors | Any unregistered hardcoded color after the second real app UI use | Promote to a named Design For AIR token before migration. One-off brand/user-content/illustration data keeps a reasoned allowlist entry. |
...
- `pnpm --filter @nn-design/web build` - passed with Next.js 16/Turbopack.
- `pnpm --filter @nn-design/web build` - passed after adding the Design For AIR `@theme` token aliases and source variables.
...
- `pnpm --filter @nn-design/web test` - passed; 99 files / 925 tests.
