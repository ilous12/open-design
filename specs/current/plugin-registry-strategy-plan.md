# Design For AIR Plugin Registry Strategy Plan
...
Turn Design For AIR plugins from "installed local workflows plus a light marketplace index" into a registry-shaped ecosystem:

- Design For AIR hosts the official catalog on the main site and ships first-party plugins there.
- Community authors publish by opening GitHub PRs into the Design For AIR registry repository.
...
- `apps/daemon/src/plugins/publish.ts` now builds submission links for external catalogs and the Design For AIR registry target. Full GitHub fork/branch/PR mutation is still future backend work.
...
1. **Official web registry**
   - Hosted on the main Design For AIR site.
   - Renders official and community-approved plugins from the Design For AIR registry repo.
   - Provides SEO pages, preview screenshots, install commands, `nd://` deep links, provenance, trust/risk metadata, and publish/contribute guidance.
...
- **Design For AIR GitHub registry repo** is the v1 storage backend. It can later be swapped for a database backend without changing user-facing nouns.
...
Design For AIR packaged runtime
...
```text
                 ...
                 public registry pages and docs
                               ...
                               v
+--------------------------------------------------+
| Design For AIR GitHub registry repo                 |
|                                                  |
...
- The file can initially live in the main Design For AIR repo if that lowers setup cost, but the product abstraction should still be `RegistryBackend`, not "read this monorepo path". Moving to `open-design/plugin-registry` later should be a data relocation, not a product rewrite.
...
- `od plugin login` wraps `gh auth login` with Design For AIR copy and required scopes/host guidance.
...
- Secondary CTA: open in desktop through `nd://plugins/<name>?source=<source>`.
...
- "Contribute to Design For AIR registry" flow with required files, checklist, CLI command, and GitHub PR path.
...
Goal: make Design For AIR contributions feel like npm publish, while actually opening a GitHub PR.
...
- [x] Add public plugin detail `nd://` deep links and static search JSON. README rendering and preview galleries remain content-quality follow-ups.
...
- Should `official` marketplace trust be allowed for user-added URLs, or only for built-in source ids shipped by Design For AIR? Recommendation: only built-in sources can be `official`; user-added sources can be `trusted` or `restricted`.
...
The architecture is already pointed in the right direction: Design For AIR can become a "multi-source plugin registry" without giving up local-first/headless operation. The next move should not be a bigger gallery. It should be provenance, trust vocabulary, and exact resolution first; then the UI can safely graduate from "official plugins plus import" to "installed and available plugins across sources."
