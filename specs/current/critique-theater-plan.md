...
```bash
pnpm --filter @nn-design/web build 2>&1 | tail -20 > /tmp/web-baseline-build.txt
```
...
```bash
pnpm --filter @nn-design/contracts test critique.test.ts
```
...
```bash
pnpm --filter @nn-design/contracts test sse.test.ts
```
...
```bash
pnpm --filter @nn-design/contracts test
```
...
import type { PanelEvent } from '@nn-design/contracts/critique';
...
```bash
pnpm --filter @nn-design/daemon test parser.test.ts
```
...
import type { PanelEvent, PanelistRole } from '@nn-design/contracts/critique';
...
import { describe, expect, it } from 'vitest';
import { defaultCritiqueConfig } from '@nn-design/contracts/critique';
...
```bash
pnpm --filter @nn-design/daemon test scoreboard.test.ts
```
...
import type { PanelistRole } from '@nn-design/contracts/critique';
...
```ts
import type { CritiqueConfig, RoundDecision } from '@nn-design/contracts/critique';
...
```ts
import type { FallbackPolicy } from '@nn-design/contracts/critique';
...
```bash
pnpm --filter @nn-design/daemon test migrations.test.ts
```
...
```bash
pnpm --filter @nn-design/daemon test orchestrator.test.ts
```
...
} from '@nn-design/contracts/critique';
import { panelEventToSse } from '@nn-design/contracts/sse';
import { parseCritiqueStream } from './parser';
...
```ts
import { describe, expect, it } from 'vitest';
import { defaultCritiqueConfig, PROTOCOL_VERSION } from '@nn-design/contracts/critique';
...
import { type CritiqueConfig, PROTOCOL_VERSION } from '@nn-design/contracts/critique';
...
```bash
pnpm --filter @nn-design/web test discovery.test.ts
```
...
import type { CritiqueSseEvent } from '@nn-design/contracts/sse';
import type { PanelistRole } from '@nn-design/contracts/critique';
