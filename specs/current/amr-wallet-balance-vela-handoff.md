...
Design For AIR needs to show the signed-in AMR wallet balance in the local client.
The accepted product decision is:

- Design For AIR displays the Vela API wallet ledger balance.
...
- A short inconsistency window is acceptable while Link has in-flight pending
  usage in Redis.
- Design For AIR must not read Link Redis directly.
- Vela API should not add Redis for this display-only feature.

This handoff is only for the Vela-side dependency needed by Design For AIR.

## Vela-Side Goal

Allow Design For AIR daemon to call the existing wallet balance endpoint with the
...
Design For AIR does not need subscription/package details for v1.
...
- No package/subscription display for Design For AIR v1.
...
as checkout creation. Design For AIR only needs the read-only balance route.
...
- The Design For AIR UI only needs account visibility.
...
- Design For AIR daemon will apply a 5-10 second in-memory TTL and singleflight to
...
## Failure Semantics Expected by Design For AIR

Design For AIR will interpret Vela responses as follows:
...
Vela does not need to create a special Design For AIR error shape. Keeping the
...
## Design For AIR Compatibility Contract

After the Vela change lands, Design For AIR daemon will call:
...
No Design For AIR behavior should depend on Link Redis, pending usage keys, or a
new Vela API cache.
...
- Response shape remains compatible with Design For AIR v1.
