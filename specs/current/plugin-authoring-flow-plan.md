...
- R2. The create entry navigates to Home, focuses `HomeHero`'s textarea, and pre-fills a prompt grounded in the Design For AIR plugin spec.
...
- No new plugin spec dialect. The generated folder must validate against the existing Design For AIR plugin shape in `docs/plugins-spec.md` and `docs/schemas/open-design.plugin.v1.json`.
...
- [x] U3. **Plugin authoring scenario**
  ...
  **Approach:**
  ...
  - The prompt should cite the Design For AIR plugin spec and require validation-ready output, not a prose-only explanation.
...
```text
Create an Design For AIR plugin for: <user goal>.
