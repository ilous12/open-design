...
test("cross-app import check rejects another app's package name", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      "import type { RunRecord } from '@nn-design/daemon/src/server.ts';",
      "const mod = await import('@nn-design/daemon');",
      "const legacy = require('@nn-design/daemon/dist/cli.js');",
    ].join("\n"),
    registry,
  );
...
test("cross-app import check rejects another app's package name in require.resolve", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "const daemonManifest = require.resolve('@nn-design/daemon/package.json');",
    registry,
  );

  assert.equal(violations.length, 1);
  assert.equal(violations[0]?.specifier, "@nn-design/daemon/package.json");
  assert.equal(violations[0]?.targetApp, "daemon");
  assert.equal(violations[0]?.lineNumber, 1);
});
...
test("cross-app import check rejects createRequire-based cross-app resolution", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      ...
      "const directDaemonManifest = createRequire(import.meta.url).resolve('@nn-design/daemon/package.json');",
      "const aliasDaemonManifest = makeRequire(import.meta.url).resolve('@nn-design/daemon/package.json');",
      "const nodeRequire = nodeModule.createRequire(import.meta.url);",
      "const daemonCli = nodeRequire('@nn-design/daemon/dist/cli.js');",
      "const daemonPackageJson = nodeRequire.resolve('@nn-design/daemon/package.json');",
    ].join("\n"),
    registry,
  );
  ...
  assert.deepEqual(
    ...
    [
      "@nn-design/daemon/package.json",
      "@nn-design/daemon/package.json",
      "@nn-design/daemon/dist/cli.js",
      "@nn-design/daemon/package.json",
    ],
  );
  assert.deepEqual(
...
test("cross-app import check rejects CommonJS node:module namespace createRequire resolution", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      ...
      'nodeRequire.resolve("@nn-design/daemon/package.json");',
    ].join("\n"),
    registry,
  );
...
test("cross-app import check rejects cross-app imports from app-owned mjs entrypoints", () => {
  ...
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "import { startDaemon } from '@nn-design/daemon/src/server.ts';",
    registry,
  );
...
test("cross-app import check allows packages, same-app relatives, and externals", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      ...
      "import type { ChatRunEvent } from '@nn-design/contracts';",
...
test("cross-app import check ignores quoted snippets and comments", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      "const snippet = \"import { x } from '@nn-design/daemon/src/server.ts';\";",
      "// import { y } from '@nn-design/daemon/src/server.ts';",
      "/*",
      "require('@nn-design/daemon/dist/cli.js');",
      "*/",
    ].join("\n"),
    registry,
  );
...
test("cross-app import check allows allowlisted packaged -> desktop main export", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "import { applyOsLocaleSwitch, createSplashWindow } from '@nn-design/desktop/main';",
    { packageNameByDirectory: new Map([["packaged", "@nn-design/packaged"], ["desktop", "@nn-design/desktop"]]) },
  );
