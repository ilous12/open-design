...
- [ ] **Run `pnpm --filter @nn-design/web test` and `pnpm --filter @nn-design/e2e test tests/localized-content.test.ts`** locally before pushing. These suites catch undisplayable discovered resources and verify localized fallback behavior.
...
- [ ] Ran `pnpm --filter @nn-design/web test` and `pnpm --filter @nn-design/e2e test tests/localized-content.test.ts`; localized-content coverage is green
