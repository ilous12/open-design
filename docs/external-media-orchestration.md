...
This note describes how an external service can use design for air as a creative
runtime while keeping media provider governance outside design for air.

design for air contributes project context, skills, design systems, previews,
...
Run-scoped `mediaExecution` controls only design for air-owned media generation.
It applies to:

- token-gated `/api/tools/media/generate`
- in-run `od media generate` when `OD_TOOL_TOKEN` is present
- design for air's Codex image generation prompt override
- design for air's media generation prompt contract
...
design for air should not grow a generic provider router, provider account pool,
...
Use HTTP/SSE between the external service and design for air. Avoid shelling out to
...
2. The external service creates or selects an design for air project.
3. The external service starts a run with `mediaExecution` set to the desired
   design for air-owned media policy.
4. The run includes skills and MCP tools that describe the external media
   workflow.
5. design for air handles design/runtime work. External MCP media tools handle
...
For runs where design for air may use only a narrow part of its own media path,
use allowlists:
...
Use design for air project context, design-system guidance, and artifact previews
to plan the campaign. When media bytes are needed, use the configured external
media MCP tools. Do not call design for air-owned media generation unless the run
...
without making design for air own provider auth or budgets.
...
the external service, not to design for air core.

The MCP server should receive its own credentials from the external service or
from its deployment environment. design for air should only see the MCP tool
...
- The external service uploads the fulfilled asset through an design for air
...
design for air project files. Project artifacts should describe creative output,
...
still available for normal design for air media generation outside the in-run tool
...
- `request-only` run mode in design for air core
- design for air media request persistence for external provider work
- a generic HTTP executor provider inside design for air
- provider credentials or account pools in design for air
- design for air-owned provider budgets, retries, or global media limits
