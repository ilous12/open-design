# Design For AIR Plugin & Marketplace Spec (v1)

> **In one sentence:** Design For AIR plugins turn portable `SKILL.md` capabilities into marketplace-ready, one-click design workflows while preserving compatibility with existing agent skill catalogs, headless CLI use, and self-hosted deployment.
...
A **Plugin** is the unit of distribution for Design For AIR. Where a [Skill](skills-protocol.md) describes a single capability that an agent can run, a Plugin is the shippable bundle around it: one or more skills, an optional design system reference, optional craft rules, optional Claude-plugin assets, a preview, a use-case query, an asset folder, and a small machine-readable sidecar that powers OD's marketplace surface. A plugin is always anchored to a portable `SKILL.md` so it is publishable to every existing skill catalog without modification.
...
| Question | Figma-era plugin assumption | Design For AIR v1 answer |
| --- | --- | --- |
...
**Current implementation clarification:** `discovery -> plan -> generate -> critique` is a reference pipeline shape, not a fixed hard-coded wizard. A plugin snapshot can carry `od.pipeline.stages[].atoms[]`; the daemon resolves that snapshot, injects the active plugin block plus active stage atom blocks into the system prompt, emits stage events, and lets the agent work through the pipeline. When the user has not explicitly selected a plugin, OD still does **not** launch a generic naked agent: the base Design For AIR designer prompt and discovery rules are always present. Product entry points bind sensible defaults on top of that base: Home free-form input routes through the bundled, hidden `od-default` scenario, while typed New Project flows choose the default bundled scenario for the project kind. `od-default` is a router and task-shaper; it should guide the run into the normal design pipeline, not be treated as a standalone "make it beautiful" aesthetic engine.
...
19. [Why this is a meaningful step for Design For AIR](#19-why-this-is-a-meaningful-step-for-open-design)
...
6. The OD core engine, atomic capabilities, and plugin runtime are all reachable from CLI so any code agent can drive Design For AIR headlessly.
...
```json
{
  ...
  "author":   { "name": "Design For AIR", "url": "https://open-design.ai" },
  "license":  "MIT",
...
- `specVersion` — the Design For AIR plugin spec version used to interpret the manifest. This is distinct from plugin `version` and is frozen into apply snapshots for replay.
...
```json
{
  ...
  "version": "1.0.0",
  "owner":    { "name": "Design For AIR", "url": "https://open-design.ai" },
...
| Frontend SDK compatibility | OD desktop / web with built-in renderer | `@nn-design/agui-adapter` lets CopilotKit / other AG-UI clients consume an OD run unchanged |
...
- A user with **only Claude Code** (or any code agent) plus `npm i -g @nn-design/cli` plus a running headless daemon can do the entire user journey: install plugin → create project → run → consume artifacts. No OD desktop required.
...
- CI is a first-class citizen: a GitHub Action can `npm i -g @nn-design/cli && od daemon start --headless && od plugin install … && od run start --project … --follow`. No display, no electron, no per-step UI scripting.
...
A code agent driving Design For AIR through the CLI typically does:
...
| Public marketplace (`open-design.ai/marketplace`)      | Anonymous visitors, SEO, share | Deep-link `nd://plugins/<id>?apply=1` (auto-installs and applies in the desktop app), plus "Copy install command" |

Deep-link contract (Phase 4 deliverable, scoped here so the schema supports it):

- `nd://plugins/<id>` — open the plugin detail in the in-app marketplace.
- `nd://plugins/<id>?apply=1[&input.k=v...]` — install if missing, then apply with the supplied inputs.
- `nd://marketplace/add?url=<urlencoded>` — register a new federated catalog.

The desktop app registers the `nd://` URL scheme; clicking a button on `open-design.ai/marketplace` either launches the desktop or, if it is not installed, falls back to a "How to install Design For AIR" flow.
...
```bash
# One-time setup: install the OD CLI as an npm global (publishable as @nn-design/cli).
npm install -g @nn-design/cli
...
Validation: `pnpm guard`, `pnpm typecheck`, `pnpm --filter @nn-design/contracts test`.
...
- `pnpm --filter @nn-design/plugin-runtime test` (parser fixtures: pure SKILL.md, pure claude plugin, metadata-only open-design.json, all three combined, SKILL frontmatter mapping).
- `pnpm --filter @nn-design/daemon test`. `pnpm guard`, `pnpm typecheck`.
...
- **§10.3.5 full AG-UI alignment:**
  - New package `@nn-design/agui-adapter` — bidirectionally maps OD's `PersistedAgentEvent` + `GenUIEvent` onto AG-UI canonical events (`agent.message`, `tool_call`, `state_update`, `ui.surface_requested`, `ui.surface_responded`).
...
| AG-UI ecosystem may evolve, drifting OD's wire format from canonical AG-UI | OD-native `GenUIEvent` remains the internal source of truth. `@nn-design/agui-adapter` is an external projection layer, so upstream protocol revs do not couple to the daemon or web renderer release cadence. |
...
- ~~**Timing of AG-UI protocol adoption**~~ — **resolved.** `@nn-design/agui-adapter` and `GET /api/runs/:runId/agui` have shipped as optional interoperability. OD-native GenUI remains the internal renderer and CopilotKit is not a required product dependency.

## 19. Why this is a meaningful step for Design For AIR
...
- `@nn-design/agui-adapter` and `/api/runs/:runId/agui` provide external AG-UI event projection without changing OD's internal GenUI renderer.
