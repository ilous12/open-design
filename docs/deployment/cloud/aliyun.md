...
This guide covers self-hosting Design For AIR on Alibaba Cloud for users in mainland China and the broader Asia-Pacific region. It documents the supported deployment paths, image-pull optimisations, and ICP filing considerations for public-facing instances.
...
For users in mainland China, deploying Design For AIR to Alibaba Cloud (阿里云) instead of an overseas provider gives you:
...
This path puts Design For AIR on a single ECS instance using the same Docker Compose stack documented in [`docs/deployment/docker.md`](../docker.md).
...
| Image | Ubuntu 24.04 LTS 64-bit | Design For AIR's Docker image is `linux/amd64` and `linux/arm64` |
...
Design For AIR does not yet ship an official Helm chart. The minimal manifest below is a starting point — production users should harden it (resource limits, PodDisruptionBudget, NetworkPolicy, persistent storage class).
...
metadata:
  name: open-design
spec:
  replicas: 1  # Design For AIR uses local SQLite; multi-replica needs shared storage
  selector:
...
Resource Orchestration Service (资源编排 ROS) provisions the same resources declaratively. There is no first-party ROS template for Design For AIR yet. Operators with Alibaba Cloud access are welcome to contribute one as a follow-up to this guide; the natural location is `deploy/aliyun/ros/`.
...
Design For AIR's daemon binds to `127.0.0.1:7456` and is never exposed directly. Public access must go through:
...
- Design For AIR Docker deployment: [`docs/deployment/docker.md`](../docker.md)
- Design For AIR one-click installer: [`docs/install-guide.md`](../../install-guide.md)
