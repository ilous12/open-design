...
This guide deploys the Docker image to Azure Container Instances (ACI) with persistent Design For AIR data. Before choosing or documenting any daemon data mount, read root `AGENTS.md` → **Daemon data directory contract**. That section is mandatory and must not be restated here.
