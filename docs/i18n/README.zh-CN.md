<h1 align="center">Design For AIR：The open-source Claude Design alternative</h1>

> 🔥 **Design For AIR 0.10.0 正式发布：All-in-one 的 Agentic 设计工作台。** 设计的全流程从此只需一个窗口——从一个模糊的想法出发，到发现参考、收集素材、交互式编辑、评论排队、打磨动效，再交付给编辑器或 Code Agent，全程无需离开应用。配合多会话并行，它不再只是一个助手，而是一支为你工作的本地设计团队。[下载 0.10.0](https://github.com/nexu-io/open-design/releases) · [参与讨论](https://github.com/nexu-io/open-design/discussions/4153)
>
> ⚡ **Design For AIR AMR（Agentic Model Router）——官方模型服务。** 一次充值，即可在 Design For AIR 里直接使用 GPT、Claude、Gemini 与 DeepSeek：20+ 旗舰模型、零配置、按真实 token 用量计费。[立即体验](https://open-design.ai/amr/)
>
> 🏅 **Design For AIR Fellow 计划正式开放。** 如果你也相信设计应该是开放的，欢迎成为 Design For AIR Fellow，和核心团队一起打磨产品，让更多人参与并定义设计的未来。详情 → [`MAINTAINERS.md`](../../MAINTAINERS.md) 与 [Discord](https://discord.gg/mHAjSMV6gz)。
...
Design For AIR 是这样一种产物：Anthropic 随 Claude Design 推出的 **Agent 原生**循环——发现需求、锁定方向、流式输出工件、评审、交付——不再封闭，而是变成了一个由**技能、设计系统和插件组成的文件系统**，你笔记本电脑上已有的编码 Agent 就能读取、编写和混搭。你的 CLI 变成设计引擎，你的笔记本变成工作坊，团队的 `DESIGN.md` 变成品牌契约。
...
快速看懂 Design For AIR 长什么样、能做什么。从 **Home** 发起创作，用 **Automation** 编排重复流程，在 **Design System** 沉淀品牌契约，靠 **Plugin** 与 **集成** 扩展能力；进入任一项目的 **Studio**，同一套设计系统即可流式产出原型、实时工件、HyperFrame、演示文稿与图片。
...
<sub><b>集成</b>——接入外部系统与 MCP 工具，把 Design For AIR 用到任意 IDE、脚本与自动化中。</sub>
</td>
</tr>
</table>
...
> Design For AIR 以 **技能、CLI 和 MCP 服务器**的形式交付，主流编码 Agent 可原生消费。装好 OD 后，一行 `od mcp install <agent>` 把 MCP 服务器 wire 进对应 Agent 的配置，任何 Agent 内调用相同工具。
...
<p align="center">
  <img src="https://repo-assets.open-design.ai/resources/images/coding-agents.png" alt="Design For AIR 支持的 21 个编码 Agent CLI — Claude Code · Codex · OpenCode · Hermes · Antigravity · Gemini · Grok Build · Kimi · Cursor Agent · Qwen · Qoder · GitHub Copilot · Pi · Kiro · Kilo · Mistral Vibe · DeepSeek · Reasonix · Aider · Devin · Trae" width="100%" />
</p>
...
**[HyperFrames][hyperframes]** 是 HeyGen 的开源 Agent 原生视频框架，在 Design For AIR 中作为一等公民集成。Agent 编写 HTML + CSS + GSAP，HyperFrames 通过 headless Chrome + FFmpeg 渲染为确定性 MP4。搭配 **Seedance 2.0** 实现影视级 t2v / i2v，**Veo 3 / Sora 2 / Kling 2** 提供路由模型变体，**Suno v5 / Lyria 2** 提供音频底座。
...
Design For AIR (OD) 是开源替代品。同样的循环，同样的工件优先心智模型，没有任何锁定：
...
使用 Design For AIR 最快的方式。无需 Node、pnpm 或克隆仓库。
...
你可以在完全不打开 GUI 的情况下使用 Design For AIR——在 Claude Code、Codex、Cursor、Copilot、OpenClaw、Antigravity、Hermes、Kimi 等中作为技能、插件或 MCP 服务器调用。
...
Sealos App Store 模板会运行已发布的 Design For AIR Docker 镜像，提供持久化工作区存储，并在公网代理层启用 Basic Auth。自定义公开或共享 Docker 部署请遵循 [`deploy/README.md`](../../deploy/README.md#local-compose) 中的反向代理和 `OPEN_DESIGN_ALLOWED_ORIGINS` 指引。
...
5. **Design For AIR 越用越聪明。** 你的截图、字体、色板和已确认的工件会累积为下次会话的默认值。更少的重复劳动，更少的偏差。

---

## 从你的编码 Agent 使用 Design For AIR

Design For AIR 提供 **stdio MCP 服务器**和逐 Agent 的**安装脚本**。任何位于其他仓库的 MCP 兼容 Agent 都可以直接读取你本地 Design For AIR 项目的文件——token CSS、JSX 组件、入口 HTML——作为按名称查询的结构化 API。Agent 始终看到实时文件，而非过期的导出。
...
**261 个官方插件**位于 [`plugins/_official/`](../../plugins/_official/)。每个插件就是一个**可移植的 agent skill 文件夹**——一个 `SKILL.md`（任何支持 Agent Skills 的 Agent 都能读），外加一个可选的 `open-design.json` manifest（给 Design For AIR 提供 marketplace 元数据、输入参数、预览、流水线与权限声明）。直接跳转到分类浏览：
...
一个插件**最小只需要一个 `SKILL.md`**；要上架 Design For AIR marketplace，再加一个 `open-design.json`：
...
1. 把插件文件夹放到 [`plugins/community/`](../../plugins/community/)（第三方插件），或——若想随 Design For AIR 一起内置——放到对应 tier 的 [`plugins/_official/`](../../plugins/_official/)。
2. 跑通校验：`od plugin validate`、`pnpm guard`、`pnpm --filter @nn-design/plugin-runtime typecheck`。
...
Design For AIR 之所以持续前进，是因为贡献者——设计师、工程师、提示词作者——不断出现。许多最常用的技能、设计系统和插件都由核心团队以外的人编写。
...
### 🏅 Design For AIR Fellow 计划

我们正在全球招募 **Design For AIR Fellow**——付费维护者，$1,000 / MR 免费 LLM 额度，官方授权在你所在地区推动增长，以及核心团队审查通道。详情 → [`MAINTAINERS.md`](../../MAINTAINERS.md) 和 [Discord](https://discord.gg/mHAjSMV6gz) 上的公告。
...
<a href="https://github.com/nexu-io/open-design/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=nexu-io/open-design&max=500&columns=20&anon=1&cache_bust=2026-06-29" alt="Design For AIR 贡献者" />
</a>
...
<picture>
  <img alt="Design For AIR——仓库指标" src="https://repo-assets.open-design.ai/resources/images/github-metrics.svg" />
</picture>
...
<p align="center">
  <a href="https://github.com/nexu-io/open-design"><img src="https://repo-assets.open-design.ai/resources/images/star-us.png" alt="在 GitHub 上给 Design For AIR 一个 Star — github.com/nexu-io/open-design" width="100%" /></a>
</p>
...
<a href="https://star-history.com/#nexu-io/open-design&Date">
  <picture>
    ...
    <img alt="Design For AIR Star 历史" src="https://api.star-history.com/svg?repos=nexu-io/open-design&type=Date&cache_bust=2026-06-29" />
  </picture>
</a>
...
| [`heygen-com/hyperframes`](https://github.com/heygen-com/hyperframes) | HTML→MP4 动态图形框架，在 Design For AIR 中作为 `hyperframes-html` 一等公民集成。 |
