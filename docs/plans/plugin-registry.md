# Design For AIR Plugin Registry — Plan (living)
...
> up their own Design For AIR plugin source with one config line.
...
start in the main Design For AIR repo, but the code path must still be expressed as
...
- `od plugin login` wraps `gh auth login` with Design For AIR copy, scopes, and
...
- [x] **P3.2 Static site renderer.** `apps/landing-page` now
  ...
  manifests, with SEO metadata, search JSON, and `nd://` detail links.
