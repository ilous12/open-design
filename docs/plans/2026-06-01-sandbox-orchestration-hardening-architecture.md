...
keep design for air safe to embed under an external orchestrator without adding
consumer-specific infrastructure to this repository.

The plan is intentionally upstream-neutral. design for air should refer to this
...
runtime contract while adding normal design for air features. The durable defense is
...
1. An external orchestrator launches design for air with `OD_SANDBOX_MODE=1` and
...
3. design for air contributes design context, skills, previews, artifacts, CLI/UI
...
Without `OD_SANDBOX_MODE=1`, design for air is the normal local product runtime, not
...
- process matching must not classify a foreign process tree as an design for air
...
- Keep the allowlist narrow so normal references to design for air, GitHub
...
- Do not add a generic provider router, provider account pool, global media
  budget system, or external executor API to design for air.
- Do not add consumer-specific conformance tests to design for air CI.
