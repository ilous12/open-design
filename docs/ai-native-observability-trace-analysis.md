...
uses Langfuse-safe manifest fields and `nd://objects/...` references rather than
...
- `pnpm install`
- `pnpm --filter @nn-design/daemon exec vitest run -c vitest.config.ts tests/langfuse-trace.test.ts tests/langfuse-bridge.test.ts tests/claude-stream-thinking.test.ts`
